...
/*   Updated: 2021/05/14 17:22:30 by mbeaujar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void decrease_buffer(t_prompt *prompt)
{
    ...
    if (prompt->buffer->strlen < ((prompt->buffer->len) / 4) && ((prompt->buffer->len) / 4) > 10)
    {
        prompt->buffer->buff = delete_char_buffer(prompt->buffer->buff, prompt->buffer->len / 4);
        prompt->buffer->len /= 4;
    }
    prompt->buffer->modified = 1;
